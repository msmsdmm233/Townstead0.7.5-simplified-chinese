 ...
 * (continuous, blended at breeding), not in the diploid {@link com.aetherianartificer.townstead.root.gene.Genotype}.
 *
...
public final class BodyMetricGeneType implements GeneType {
    ...
    private static final String LOCUS_NS = "townstead_roots";

    /** {@code target} is the normalized MCA gene key (see {@link RootGenes#normalizeKey}). */
    ...
    public GeneInstance parse(JsonObject json) {
        String target = RootGenes.normalizeKey(GsonHelper.getAsString(json, "target", ""));
        if (target.isEmpty() || !RootGenes.isKnown(target)) return null;
